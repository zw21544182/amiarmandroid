        ...
        for (SystemConfig config:listConfig
            ...
            switch(config.getParamName())
            {
                ...
                case "FloatMax":
                    ...
                    mapConfig.put(config.getParamName(),config.getParamValue());
                    break;
                case "FiberMax":etFiberMaxCount.setText(String.valueOf(config.getParamValue()));
                    mapConfig.put(config.getParamName(),config.getParamValue());
                    break;
                case "GlassCountMax":etGlassPrecentMax.setText(String.valueOf(config.getParamValue()));
                    mapConfig.put(config.getParamName(),config.getParamValue());
                    break;
                case "GlassTimeMax":etGlassTimeMax.setText(String.valueOf(config.getParamValue()));
                    mapConfig.put(config.getParamName(),config.getParamValue());
                    break;
                case "DiskUsedMax":etDiskPrecentMax.setText(String.valueOf(config.getParamValue()));
                    mapConfig.put(config.getParamName(),config.getParamValue());
                    break;
                case "IpAddress":etIpAddress.setText(String.valueOf(config.getParamValue()));
                    mapConfig.put(config.getParamName(),config.getParamValue());
                    break;
                case "NetMask":etNetMask.setText(String.valueOf(config.getParamValue()));
                    mapConfig.put(config.getParamName(),config.getParamValue());
                    break;
                case "GateWay":etGateWay.setText(String.valueOf(config.getParamValue()));
                    mapConfig.put(config.getParamName(),config.getParamValue());
                    break;
                default:
                    break;
    ...
    private void SaveData()
    {
      ...
      mapConfig.put("GlassCountMax", etGlassPrecentMax.getText().toString());
        ...
        try {
            ...
            else Toast.makeText(mActivity, String.format("总项目%d条,成功保存%d条",count,success),Toast.LENGTH_SHORT).show();
        } catch (RemoteException e) {
            e.printStackTrace();
        }
